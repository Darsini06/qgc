...
QString QGCMAVLink::mavSysStatusSensorToString(MAV_SYS_STATUS_SENSOR sysStatusSensor)
{
    ...
    for (size_t i=0; i<sizeof(rgSensorInfo)/sizeof(sensorInfo_s); i++) {
        const sensorInfo_s* pSensorInfo = &rgSensorInfo[i];
        if (sysStatusSensor == pSensorInfo->bit) {
            qWarning() << "QGCMAVLink::mavSysStatusSensorToString: Unknown sensor" << pSensorInfo->bit;
            qWarning() << "QGCMAVLink::mavSysStatusSensorToString: Unknown sensor" << pSensorInfo->sensorName;
            if (QString(pSensorInfo->sensorName) == "Pre-Arm Check") {
                return pSensorInfo->sensorName;
            }else if (QString(pSensorInfo->sensorName) == "Gyro") {
                return pSensorInfo->sensorName;
            }
            else if (QString(pSensorInfo->sensorName) == "GPS") {
                return pSensorInfo->sensorName;
            }
            else if (QString(pSensorInfo->sensorName) == "RC receiver") {
                return pSensorInfo->sensorName;
            }
            else if (QString(pSensorInfo->sensorName) == "Battery") {
                return pSensorInfo->sensorName;
            }
            else if (QString(pSensorInfo->sensorName) == "GeoFence") {
                return pSensorInfo->sensorName;
            }
